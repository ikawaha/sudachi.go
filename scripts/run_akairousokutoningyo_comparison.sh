...
for MODE in A B C; do
    GOLDEN_FILE="$TEST_DIR/akairousokutoningyo_golden_mode_${MODE}.txt"
    if [ ! -f "$GOLDEN_FILE" ]; then
        echo "❌ ゴールデンデータが見つかりません: $GOLDEN_FILE"
        echo "    まず scripts/generate_akairousokutoningyo_golden.sh を実行してください"
        exit 1
    else
...
# テスト実行
echo ""
echo "🚀 赤いろうそくと人魚全文比較テスト実行開始..."
echo "   対象: $TOTAL_SENTENCES 文 × A/B/C 3モード"
echo ""

# テスト実行（詳細ログ出力）
go test -v ./test/regression -run TestAkairousokutoningyoFullComparison -timeout 30m
...
# レポートファイルの確認
REPORT_FILE="$TEST_DIR/akairousokutoningyo_full_comparison_report.json"
